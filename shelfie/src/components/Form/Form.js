...
class Form extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      img:
        props.img ||
        "https://images.unsplash.com/photo-1543349823-729e19b44e31?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1350&q=80",
  ...
  render() {
    ...
    const buttonCheck = () => {
      if (this.state.selectedProduct === null) {
        return (
          <button onClick={event => this.props.addProduct(productInfo)}>
            Add to Inventory
          </button>
        );
      } else {
        return (
          <button
            onClick={event => this.updateProduct(productInfo, selectedProduct)}
          >
            Save Changes
          </button>
        );
    ...
    return (
      <div>
        Form
        <img className="daImage" src={this.state.img} />
        <input
